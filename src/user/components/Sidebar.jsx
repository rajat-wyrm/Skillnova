...
const MENU = [
  ...
  // Your User Progress Tracker feature
  { id: 'progress_tracker', label: 'My Progress', icon: BarChart2 },
...
const Sidebar = ({ active, onNavigate, forceMobileExpanded }) => {
  ...
  return (
    <aside
      className={`h-screen flex flex-col flex-shrink-0 transition-all duration-300 ${
        isCollapsed
          ? 'w-16'
          : 'w-60'
      }`}
    ...
    >
      ...
      <div
        className={`h-20 flex items-center gap-3 flex-shrink-0 ${
          isCollapsed
            ? 'px-3 justify-center'
            : 'px-4'
        }`}
      ...
      <nav
        className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5 no-scrollbar"
        role="navigation"
