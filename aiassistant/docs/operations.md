# AIAssistant — operations runbook
...
- `llm_configured=false` → no GROQ/GEMINI/DEEPSEEK key in env. Add one
  in `aiassistant-backend/.env` and restart the service. Until then,
  the chat endpoints will return a 503 with a clear error.
...
| Concern | File |
| --- | --- |
| LLM provider chain | `aiassistant-backend/llm.py` |
| Circuit breaker state | `CircuitBreaker.can_execute()` |
| Structured logs | `aiassistant-backend/logs/app.log` (line per JSON record) |
| Request ID | `request_id_var` in `logging_config.py` |
| Guardrail patterns | `BLOCKED_PATTERNS` in `aiassistant-backend/guardrails.py` |
| Cache backend | `create_cache()` in `aiassistant-backend/cache.py` |
| Knowledge base files | `aiassistant-backend/knowledge_base/*.txt` |
...
```
rm aiassistant-backend/chat_logs.db
```
...
# Frontend
cd "aiassistant/aiassistant-widget"
npm install
npm run dev
```
