# Architecture — SkillNova AIAssistant

This document explains how the **AIAssistant** fits into the wider
SkillNova platform without modifying the existing Node/Express backend
or React frontend.

## Goals

1. Zero changes to the existing Skillnova core code.
2. Fully self-contained Python service with its own Dockerfile.
3. One single chat surface (the AIAssistant) that's reachable from
...
| Path | Purpose |
| --- | --- |
| `aiassistant-backend/main.py` | FastAPI app, request lifecycle |
| `aiassistant-backend/agent/` | LangGraph nodes + graph definition |
| `aiassistant-backend/retriever/` | FAISS index build + similarity search |
| `aiassistant-backend/tools/web_search.py` | Tavily / DuckDuckGo fallback |
| `aiassistant-backend/llm.py` | Multi-provider LLM calls (circuit-broken) |
| `aiassistant-backend/cache.py` | In-memory + Redis semantic cache |
| `aiassistant-backend/database.py` | SQLAlchemy models + chat persistence |
| `aiassistant-backend/guardrails.py` | Input validation + language detection |
| `aiassistant-backend/logging_config.py` | Structured JSON logger |
| `aiassistant-backend/knowledge_base/` | Plain-text KB with YAML frontmatter |
| `aiassistant-widget/` | Standalone React widget (Vite) |

## Request lifecycle

```
client → POST /api/aiassistant/chat
    │
    ▼
...
The widget sends `user_role` on every request (one of
`SUPER_ADMIN`, `ADMIN`, `MENTOR`, `INTERN`). The AIAssistant tailors
answers to the asker, e.g. an intern asking about payroll gets a
different answer than an admin asking the same thing. The role is
inferred from the SkillNova auth store on the client side; the backend
trusts the value but does not re-validate it (the API is currently
open within the cluster).
...
Instead we expose the AIAssistant as a standalone API and treat it
like any other upstream the frontend already talks to (e.g. the
existing Groq client in `server/src/ai/assistant.service.js`).
