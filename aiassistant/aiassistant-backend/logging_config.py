"""Structured logging for the SkillNova AIAssistant backend.
...
def setup_logging() -> logging.Logger:
    """Configure the root ``aiassistant`` logger.

    Creates ``logs/`` and ``logs/app.log`` if missing. Idempotent — safe to
    call multiple times (handlers are deduped by the ``aiassistant``
    namespace not propagating to root).
    """
    ...
    root = logging.getLogger("aiassistant")
