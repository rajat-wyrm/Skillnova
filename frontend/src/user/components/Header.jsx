      ...
      try {
        ...
        const mapped = list.slice(0, 5).map(item => ({
          ...
          time: item.date
            ...
            : "Recently",
          // Prioritised announcements get the orange dot, others green
    ...
    >
                ...
                notifications.map((n, i) => (
                  ...
                  >
                    <div
                      ...
                      style={{ background: n.type === "primary" ? "#ff6d34" : "#00bea3" }}
                    />
                    <div>
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 pl-2 sm:pl-3" style={{ borderLeft: "1px solid var(--border)" }}>
        ...
        <div className="hidden md:block">
          <p className="text-xs font-semibold leading-none" style={{ color: "var(--text)" }}>{user?.name || "User"}</p>
          <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>{user?.title || user?.role || "Intern"}</p>
        </div>
      </div>
    </header>
  );
